...
// Schools by League exported so it can be used by direct import
export const leagueSchools: Record<string, SchoolData[]> = {
  // 6A Districts
  '6a-1': [
...
export const importSchoolsAndTeams = async () => {
  ...
  console.log('Starting school and team import...');
  
  // Log the Supabase client status to help debug
  console.log('Supabase auth status:', supabase.auth);
  ...
  if (checkError) {
    ...
    importLogs.push(`Database read error: ${checkError.message}`);
    throw checkError;
  }
  
  console.log('Checked for existing schools, found:', existingSchoolsData?.length || 0);
  
  // We'll proceed with import regardless if schools exist or not
  const shouldOnlyAddNew = true; // Always check for duplicates
  
  for (const [districtCode, schools] of Object.entries(leagueSchools)) {
    console.log(`Processing district ${districtCode} with ${schools.length} schools`);
    ...
    if (districtError) {
      ...
      importLogs.push(`Error finding district ${districtCode}: ${districtError.message}`);
      continue;
    }
    ...
    for (const school of schools) {
      ...
      try {
        ...
        if (schoolCheckError) {
          ...
          importLogs.push(`Error checking for ${school.name}: ${schoolCheckError.message}`);
          continue;
        }
        ...
        // Log some detailed information
        console.log(`Preparing to insert school: ${school.name} with district_id: ${districtData.id}`);
        ...
        if (schoolError) {
          ...
          importLogs.push(`Failed to add ${school.name}: ${schoolError.message}`);
          continue;
        }

        schoolsAdded++;
        console.log(`Added school: ${school.name} with ID: ${schoolData.id}`);
        ...
        if (teamError) {
          ...
          importLogs.push(`Failed to add teams for ${school.name}: ${teamError.message}`);
        } else {
          console.log(`Added ${teamsData.length} teams for ${school.name}`);
          teamsAdded += teamsData.length;
        }
      } catch (error) {
        // Catch any other errors
        console.error(`Unexpected error processing school ${school.name}:`, error);
        importLogs.push(`Error with ${school.name}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }

  console.log('School and team data import completed with summary:', {
    schoolsAdded,
    teamsAdded,
    existingSchools,
    logs: importLogs
